...
def train_network(q, q_target, memory, optimizer, epsilon, args):
    loss_list = []
    ...
    for i in range(10):
        ...
        q_a = q(s).gather(1, a)
        with torch.no_grad():
            # current policy is argmax(a) Q(s'), it's not different from Q-learning
            a_prime = q_target(s_prime).argmax(1).unsqueeze(-1)
            q_a_prime = q_target(s_prime).gather(1, a_prime)

        target = r + args.gamma * q_a_prime * done_mask
...
def train(args):
    ...
    for ep in range(args.max_epochs):
        ...
        if memory.size() > args.init_steps:
            loss = train_network(q, q_target, memory, optimizer, epsilon, args)
        else:
            loss = 0.0
...
if __name__ == '__main__':
    ...
    p.add_argument('--device', type=str, choices=devices, default=devices[0])
